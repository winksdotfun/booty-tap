...
function App() {
  ...
  const [lastActionCoordinates, setLastActionCoordinates] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const checkActionArea = (x: number, y: number) => {
    const actionAreas = [
      { id: "area1", x: 153, y: 322, width: 223, height: 105 },
    ];
  ...
  useEffect(() => {
    if (
      level - 1 < levelRequirements.length &&
      score >= levelRequirements[level - 1]
    ) {
      setLevel((prevLevel) => {
